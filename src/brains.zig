...
fn executeBrainHero(
    ...
    if (opt_head) |head| {
        if (opt_body) |body| {
            ...
            var acceleration_normal: Vector3 = .zero();
            var acceleration: Vector3 = controlled_hero.controller_direction.toVector3(0);
            const acceleration_length = acceleration.lengthSquared();
            if (acceleration_length > 0.05) {
                const normalize_coefficient: f32 = 1.0 / intrinsics.squareRoot(acceleration_length);
                acceleration_normal = acceleration.scaledTo(normalize_coefficient);
                if (acceleration_length > 1.0) {
                    acceleration = acceleration.scaledTo(normalize_coefficient);
                }
            }

            const inverse_clutch: f32 = 1.0 - clutch_level;
            const hop_requested: bool = !dodging and body.movement_mode == .Planted and acceleration_length > 0.5;
            ...
            const clutch_bob: f32 = 0.25 * inverse_clutch;
            if (dodging) {
                body.bob_time = clutch_bob;
            }
