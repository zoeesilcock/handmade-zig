...
pub const IHeader = extern struct {
    ...
    compression_method: u8 align(1),
    filter_method: u8 align(1),
    interlace_method: u8 align(1),
};

pub const IDataHeader = extern struct {
    zlib_method_flags: u8 align(1),
    additional_flags: u8 align(1),
};
...
// This function is derived from the sample in the PNG spec:
// https://www.w3.org/TR/png-3/#D-CRCAppendix
fn endBigCRC(skip_n_in: u32, start_chunk: ?*StreamChunk, one_past_last_chunk: ?*StreamChunk) u32 {
    var result: u32 = 0xffffffff;
    var skip_n: u32 = skip_n_in;
    var opt_chunk: ?*StreamChunk = start_chunk;
    while (opt_chunk != one_past_last_chunk) {
        const chunk = opt_chunk.?;
        defer opt_chunk = chunk.next;

        std.debug.assert(skip_n <= chunk.contents.count);

        var n: usize = skip_n;
        while (n < chunk.contents.count) : (n += 1) {
            result = global_crc_table[(result ^ chunk.contents.data[n]) & 0xff] ^ (result >> 8);
        }
        skip_n = 0;
    }
    ...
    if (at.consumeType(Header)) |header| {
        ...
        while (at.contents.count > 0) {
            if (at.consumeType(ChunkHeader)) |chunk_header| {
                ...
                // Casey's version has this, but our version fails to read PNGs with this.
...
pub fn writePNG(
    ...
    var start_crc: ?*StreamChunk = null;
    _ = stream.outputStructCopy(out, &Signature);
    ...
    var chunk_header: ChunkHeader = .{
        .length = @sizeOf(IHeader),
        .chunk_type = @bitCast(fourcc("IHDR")),
    };
    var chunk_footer: ChunkFooter = .{ .crc = 0 };

    endianSwap(&chunk_header.length);
    // Casey's version has this, but our version creates invalid PNGs with this.
    // chunk_header.endianSwapType();

    _ = stream.outputStructCopy(out, &chunk_header);
    start_crc = out.last;
    _ = stream.outputStructCopy(out, &ihdr);
    chunk_footer.crc = endBigCRC(4, start_crc, null);
    _ = stream.outputStructCopy(out, &chunk_footer);

    // IDAT.
    var idat: IDataHeader = .{
        .zlib_method_flags = 8,
        // The FCHECK value has to make the IDAT be a multiple of 31, so 29 corrects for the fact that we would have
        //a remainder of 2 when we set zlib_method_flags to 8.
        .additional_flags = 29,
    };
    ...
    chunk_header.chunk_type = @bitCast(fourcc("IEND"));
